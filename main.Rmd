...
```{r}
load("dados/2020_2021_raw.RData")
load('dados/index_metiers.rdata')
...
```{r}
classe_modal_cmp = amp_cmp %>% 
  group_by(ANO, MES, GEAR, zona) %>% 
  summarise(classe_modal = classe_comp[which.max(freq)],
            freq = max(freq))
```
